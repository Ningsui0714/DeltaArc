...
当前版本最主要的问题不是“功能太少”，而是下面这 4 个结构风险：

- `shared` 里只有类型，没有运行时协议。
- 推演主链集中在一个超大 orchestrator 文件。
- 项目、证据、记忆分散在浏览器和服务端两套状态源里。
- 缺少固定样本回归，AI 很容易改着改着把语义改歪。
...
一期的 schema 目标不是立刻引入复杂框架，而是把当前已经分散在前端导入层、后端 normalize 层里的规则，逐步收口到 `shared/schema/`。
...
### 3. `lastAnalysisAt` 不再继续挂在 `ProjectSnapshot` 当真相

当前 `ProjectSnapshot.lastAnalysisAt` 更像 UI 衍生字段，不应该继续承担正式项目状态。
...
如果只有 `examples/` 而没有统一校验入口，样本很快会变成摆设。
